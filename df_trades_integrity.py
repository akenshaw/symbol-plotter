                ...
                async with session.get(url, params=params, headers=headers) as response:
                    ...
                    if response.status == 200:
                        ...
                        if len(trades) == 0:
                            ...
                            break
                        if trades[0]["id"] >= next_trade_id:
                            print(f"Before filtering: {trades}")
                            trades = [trade for trade in trades if trade["id"] < next_trade_id]
                        ...
                        data.extend(trades)
                        last_trade_id = trades[0]["id"] + len(trades)
...
async def main(symbol, df):
    print(f"Total trades before the integrity check is {len(df)}. Continuing for missing trades in {symbol}...")
    while True:
        try:
    ...
    print(f"Total trades after the integrity check is {len(df)}\n")
    return df
# %%
