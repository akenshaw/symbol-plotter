    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params, headers=headers) as response:               
            if response.status == 200:
                ...
                elif used_weight > 2000:
                    print(f"\ncool down triggered with {used_weight}, {task_name}, sleeping for 10 seconds...")
                    await asyncio.sleep(10)
                elif used_weight > 1400:
...
async def fetch_trades(symbol, date, recent_df_exists, start_trade_id):
    ...
    df_checked.to_csv(f"/Users/berkes/new_save/recent_trades_df/{symbol}_recent_trades_{date}.csv", index=False)
    return df_checked
