...
from df_trades_integrity import main as integrity 
...
async def get_trades_in_range(symbol, start_from_id, num_trades):
    ...
    while remaining_trades > 0:
        ...
        last_trade_time = batch_trades[-1]["time"]
...
async def main(symbol=None, from_date=None, recent_df_exists=False, start_trade_id=None):
    ...
    file_handler = logging.FileHandler(f"/Users/berkes/new_save/logger/{symbol}_recent_trades.log")
    logger.addHandler(file_handler)
    file_handler.setFormatter(formatter)
    
    print(f"Starting recent trades fetch... -> from {from_date}, with a fromId of {start_trade_id}")
    tasks = [
